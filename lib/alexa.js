        ...
        return {
            ...
            'audioPlayer': function (directiveType, behavior, url, token, expectedPreviousToken, offsetInMilliseconds) {
                ...
                if (directiveType === 'play') {
                    ...
                    // allow chaining play directives (but don't chain on top of a non-play directive)
                    audioPlayerDirectives = responseObject.response.directives;
                    if (audioPlayerDirectives && audioPlayerDirectives.length > 0 && audioPlayerDirectives[0].type === "AudioPlayer.Play")
                        audioPlayerDirectives.push(playDirective);
                    else
                        audioPlayerDirectives = [playDirective];
                ...
                responseObject.response.directives = audioPlayerDirectives;
                return this;
            },
            'audioPlayerPlay': function (behavior, url, token, expectedPreviousToken, offsetInMilliseconds) {
                return this.audioPlayer('play', behavior, url, token, expectedPreviousToken, offsetInMilliseconds);
            },
            'audioPlayerStop': function () {
                return this.audioPlayer('stop');
            },
            'audioPlayerClearQueue': function (clearBehavior) {
                return this.audioPlayer('stop', clearBehavior);
            }
        }
    })();
}
